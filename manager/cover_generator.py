"""
Auto-generate stylish book cover images when no cover is uploaded.
Design matches the existing generated covers: colorful card with white border,
book title centered, BESTSELLER badge, and "精品图书" label.
"""
import os
import random
import textwrap
...
def _wrap_text(text, max_chars=14):
    lines = textwrap.wrap(text, width=max_chars)
    return lines if lines else [text]


def generate_cover_image(book_name, book_id):
    """
    Generate a cover image for a book and return the relative path
    (suitable for saving to book.cover_image field).
    Returns the relative media path like 'book_covers/gen_cover_123_xxx.png'.
    """
    cover_dir = os.path.join(settings.MEDIA_ROOT, 'book_covers')
    os.makedirs(cover_dir, exist_ok=True)

    card_color, card_color_dark = random.choice(COLOR_PALETTE)
    ...
    title_lines = _wrap_text(book_name, max_chars=14)
    line_height = 32
    ...
    # BESTSELLER Badge
    badge_font = _get_font(11, bold=True)
    ...
    # Save
    safe_name = ''.join(c if c.isalnum() or c in '-_ ' else '' for c in book_name)[:30].strip()
    filename = f'gen_cover_{book_id}_{safe_name}.png'
